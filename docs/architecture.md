...
- `mynotes:draft:{id}` — черновик незасохранённого редактирования.
...
Пустой заголовок допустим, в списке — «Без названия». Пустой пункт не добавляется; пустой текст после blur удаляет пункт.
